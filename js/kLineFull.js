...
var KLineRequire = function(option, klineType){
    ...
    if(klineType=="day"){
        ...
        if(klineType=="week"||klineType=="month"||klineType=="year"){
            // 更新查询历史数据参数
            this.options.HistoryKQAll = $.extend({}, this.options.HistoryKQAll, { MsgType: objKWatchMin.MsgType});
        }else{
            // 更新查询历史数据参数
...
var initSocketEvent = function(socket){
    ...
    socket.ws.onopen = function () {
                    ...
                    if(socket.option.lineType&&KLineSocket.option.lineType!="mline"){
                        ...
                        if(o!==""){
                            ...
                            switch(MsgType){
                                case "P0001":       // 订阅快照
                                    ...
                                    // K线接口
                                    if(socket.option.lineType=="day"&&socket.HistoryData.hCategoryList.length>0){
                                        KCharts(dataList);
                                    }
                                    break;
